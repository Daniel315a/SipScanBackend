...
def build_key(uploader_nit: str, filename: str | None, content_type: str | None) -> str:
    ...
    safe_nit = "".join(
        ch for ch in (uploader_nit or "") if ch.isalnum() or ch in ("-", "_")
    )
...
async def upload_image(upload_file, uploader_nit: str) -> tuple[str, str]:
    ...
    """
    key = build_key(
        uploader_nit, getattr(upload_file, "filename", None), upload_file.content_type
    )
    s3 = _client()
...
def presign_put_url(
    bucket: str, key: str, ttl: int | None = None, content_type: str | None = None
) -> str:
    """Generate a time-limited URL for PUT uploads (client-side upload).

    If `content_type` is provided, include it in the params so the client can
